...
async function updatePlayerRanking(update: EloUpdate, won: boolean): Promise<void> {
    return new Promise((resolve, reject) => {
...
async function saveRankedMatch(
    ...
    return new Promise((resolve, reject) => {
        const winnerId = score1 > score2 ? player1.sub : score2 > score1 ? player2.sub : null;
...
// Matchmaking Logic
// ─────────────────────────────────────────────────────────────────────────────
function findRankedMatch(newPlayer: RankedWaitingItem): RankedWaitingItem | null {
    ...
    const sortedWaiting = rankedWaiting
        ...
        .sort((a, b) => Math.abs(a.payload.elo - newPlayer.payload.elo) - Math.abs(b.payload.elo - newPlayer.payload.elo));

    for (const opponent of sortedWaiting) {
        const eloDifference = Math.abs(opponent.payload.elo - newPlayer.payload.elo);
...
export async function handleRankedStart(
    ...
    try {
        ...
        if (!rankedPlayer) {
            socket.send(
                JSON.stringify({ type: "error", message: "Failed to load player ranking" })
            );
            return;
        }
...
async function handleRankedMatchEnd(session: RankedSession): Promise<void> {
    try {
        ...
        console.log(`Ranked match ${session.id} completed. ELO changes:`, eloChanges);
    } catch (error) {
...
// ─────────────────────────────────────────────────────────────────────────────
export async function handleRankedForfeit(session: RankedSession, disconnectedSocket: WebSocket, reason: string = "disconnection"): Promise<void> {
    try {
        // Determine who disconnected and who wins
        const disconnectedPlayer = disconnectedSocket === session.sockets.p1 ? 'p1' : 'p2';
        const winner = disconnectedPlayer === 'p1' ? 'p2' : 'p1';

        // Set forfeit score (10-0 for the remaining player)
        session.game.forfeit(winner);

        console.log(`Player ${disconnectedPlayer} forfeited due to ${reason}. Winner: ${winner}`);
        ...
        try {
            ...
            console.log("Could not notify remaining player - they may have also disconnected");
        }
...
export async function cleanupRankedSocket(socket: WebSocket): Promise<void> {
    ...
    const waitingIndex = rankedWaiting.findIndex((item) => item.socket === socket);
    if (waitingIndex >= 0) {
    ...
    if (session) {
        ...
        if (!session.game.isGameOver) {
            console.log(`Player disconnected during active ranked match: ${session.id}`);
