    ...
    app.post(
        ...
        async (
            ...
            try {
                ...
                // Get the user's Ethereum address from database
                const getUserAddress = (): Promise<string> => {
                    return new Promise((resolve, reject) => {
                        db.get(
                            ...
                            (err, row: any) => {
                                if (err) reject(err);
                                else if (row && row.address) resolve(row.address);
                                else reject(new Error('User address not found'));
                            }
                        );
                    });
                };
                ...
                const txHash = await postScore(gameId, userAddress, score);
                reply.send({ txHash });
            } catch (err: any) {
