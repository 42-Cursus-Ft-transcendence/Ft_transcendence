...
export function renderBlockExplorer(
  ...
  const searchInput =
    container.querySelector<HTMLInputElement>("#searchInput")!;
  const latestBlocksContainer =
    container.querySelector<HTMLElement>("#latestBlocks")!;
  const tournamentScoresContainer =
    container.querySelector<HTMLElement>("#tournamentScores")!;
  const blockModal = container.querySelector<HTMLElement>("#blockModal")!;
  const closeModalBtn =
    container.querySelector<HTMLButtonElement>("#closeModal")!;
  const blockDetailsContainer =
    container.querySelector<HTMLElement>("#blockDetails")!;

  // Stats elements
  const latestBlockNumberEl =
    container.querySelector<HTMLElement>("#latestBlockNumber")!;
  const totalTournamentsEl =
    container.querySelector<HTMLElement>("#totalTournaments")!;
  ...
  async function renderLatestBlocks(): Promise<void> {
    ...
    latestBlocksContainer.innerHTML = currentBlocks
      ...
      .map((block) => {
        ...
        return `
        <div class="bg-black/30 border ${blockClass} rounded p-2 cursor-pointer block-item transition" data-block='${JSON.stringify(
          block
        )}'>
          <div class="flex justify-between items-center mb-1">
            <div class="text-accent font-arcade text-xs">${blockLabel}</div>
            <div class="text-green-300 text-xs">${formatTimestamp(
              block.timestamp
            )}</div>
          </div>
          ...
          <div class="flex justify-between text-xs">
            ...
            <span class="text-blue-300">${parseInt(
              block.gasUsed || "0"
            ).toLocaleString()} gas</span>
          </div>
        </div>
      `;
      })
  ...
  async function renderTournamentScores(): Promise<void> {
    ...
    tournamentScoresContainer.innerHTML = currentTournaments
      ...
      <div class="bg-black/30 border border-pink-400 hover:border-pink-500 rounded p-2 transition">
        <div class="flex justify-between items-center mb-1">
          <div class="text-accent font-arcade text-xs">Game ${
            tournament.id
          }</div>
          <div class="text-green-300 text-xs">${formatTimestamp(
        ...
        <div class="mb-1">
          ...
          <span class="text-green-400 text-xs"> Score: ${
            tournament.score
          }</span>
        </div>
  ...
  function showBlockDetails(block: Block): void {
    blockDetailsContainer.innerHTML = `
      <div class="space-y-3">
        <div><span class="text-accent">Block Number:</span> ${
          block.number
        }</div>
        <div><span class="text-accent">Hash:</span> ${block.hash}</div>
        <div><span class="text-accent">Timestamp:</span> ${formatTimestamp(
          block.timestamp
        )}</div>
        <div><span class="text-accent">Transactions:</span> ${
          block.transactions
        }</div>
        <div><span class="text-accent">Gas Used:</span> ${parseInt(
          block.gasUsed
        ).toLocaleString()} / ${parseInt(block.gasLimit).toLocaleString()}</div>
        ...
        <div class="mt-4 pt-3 border-t border-accent/30">
          ...
          <div class="bg-black/50 rounded h-4 border border-accent/30">
            <div class="bg-accent h-full rounded" style="width: ${
              (parseInt(block.gasUsed) / parseInt(block.gasLimit)) * 100
            }%"></div>
          </div>
  ...
  async function handleSearch(): Promise<void> {
    ...
    if (query.startsWith("0x")) {
      ...
      alert(
        "Invalid search query. Please enter a transaction hash (0x...) or block number."
      );
    }
  ...
  function initializeBlockExplorer(): Promise<void> {
