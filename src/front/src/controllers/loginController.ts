...
function show2FAVerification(
  ...
  return new Promise((resolve) => {
    ...
    const codeInputs = currentContainer.querySelectorAll<HTMLInputElement>(
      '#twofaVerifyForm input[type="text"]'
    );
    const errorElement = currentContainer.querySelector(
      "#error-2fa"
    ) as HTMLElement;
    const loginLink = currentContainer.querySelector(
      'a[href="#login"]'
    ) as HTMLElement;
